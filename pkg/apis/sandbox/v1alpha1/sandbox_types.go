...
type SandboxStatus struct {
	...
	// Token is what execd was told to honour for this sandbox, in the clear.
	//
	// No tenant ever sees it. Callers authenticate to the data plane with the
	// tenant's API key, and the gateway swaps this in for the hop into the Pod,
	// so execd keeps a credential it can verify without reaching a database it
	// must never be able to reach.
	//
	// Reading it needs get on sandboxes in the tenant's namespace, which is not
	// something a tenant has. That is the same bargain the Pod's control-plane
	// token already makes (see controller/podspec.go), and it is cleared the

...
嵌套挂载已支持（原第一版限制已解除）。`Generate` 按深度重排，父在子前——bwrap 按 argv 顺序施加、后者覆盖前者，子在前会被父静默盖掉。重排确定（深度，然后路径），因为 argv 进入模板 hash。

已实测的两种形状：可写父 + 只读子（agent 的 sessions 目录），只读父 + 可写子。硬约束是嵌套挂载不能凭空造挂载点：目录必须在父的源里已存在；父可写时 bwrap 会 mkdir 并真的写进父的源，父只读时 bind 失败，错误信息现在直接给出 bwrap 那一行原话。

仍未做：只读源本身带有未知子挂载时的递归只读。当前的卷是单个 NFS 挂载，源目录下不会有子挂载，所以这条路走不到；换成会出现子挂载的卷之前必须先解决。
